...
默认输出目录可通过本机 `.env` 的 `CMD_OUTPUT_DIR` 设置，例如指向个人 Obsidian 仓库；未配置时使用项目的 `outputs`。网页仍可选择默认目录、桌面、文档、下载或输入任意自定义路径；终端菜单也会在转换前询问输出文件夹。转换完成后页面会显示绝对路径、文件清单，并提供打开文件夹和完整 ZIP 下载。不要把含个人路径或密钥的 `.env` 提交到 GitHub。

Windows 用户还可以运行 `launch_app.ps1`，或使用安装时创建的桌面快捷方式，一键启动服务并打开网页。
